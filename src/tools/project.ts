    ...
    "Deploy source code from a Salesforce project to an org. This command must be run from within a Salesforce DX project directory. Use various flags to control what gets deployed (manifest, metadata, source-dir) and how (dry-run, test level).",
    {
      input: z.object({
        sourcePath: z
          .string()
          .describe(
            "Absolute path to the Salesforce DX project directory containing .sf directory with org authentication"
          ),
        dryRun: z
          .boolean()
          .describe(
        ...
        targetOrg: z
          ...
          .optional()
          .describe(
            "Username or alias of the target org. If not specified, uses the default org configured in the project."
          ),
      }),
    },
    async ({ input }) => {
