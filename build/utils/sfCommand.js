...
function findSfPath() {
    ...
    const pathsToCheck = COMMON_SF_PATHS[currentPlatform] ||
        ...
        exec(fullCommand, { maxBuffer: 50 * 1024 * 1024 }, (error, stdout, stderr) => {
            ...
            if (error) {
                if (error.message.includes("command not found") ||
                    ...
                    reject(new Error("Salesforce CLI (sf) not found. Please ensure it is installed and accessible. " +
                ...
                try {
                    ...
                    const detailedMessage = `Error: ${errorDetails.message}\n` +
...
export function executeSfCommandRaw(command) {
    const sfPath = findSfPath();
    const fullCommand = command.replace(/^sf\s+/, `"${sfPath}" `);
    return new Promise((resolve, reject) => {
        exec(fullCommand, { maxBuffer: 50 * 1024 * 1024 }, (error, stdout, stderr) => {
 ...
 */
export function getDefaultUsername(projectPath) {
    try {
...
export function validateProjectPath(projectPath) {
    ...
    if (!existsSync(sfConfigPath)) {
        throw new Error(`Project path does not contain .sf directory. ` +
 ...
 */
export function executeSfCommandInProject(command, projectPath) {
 ...
 */
export function executeSfCommandInProjectRaw(command, projectPath) {
    // Validate project path first
    validateProjectPath(projectPath);
    const sfPath = findSfPath();
    const resolvedPath = resolve(projectPath);
