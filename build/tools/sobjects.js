...
const executeSobjectList = async (sourcePath, targetOrg) => {
    const sfCommand = buildSfCommand(`sf sobject list --sobject all`, targetOrg, sourcePath);
    try {
...
const executeSObjectDescribe = async (sourcePath, sObjectName, targetOrg) => {
    const sfCommand = buildSfCommand(`sf sobject describe --sobject ${sObjectName}`, targetOrg, sourcePath);
    try {
        const result = await executeSfCommandInProject(sfCommand, sourcePath);
        return result;
    }
...
export const registerSObjectTools = (server) => {
    ...
    server.tool("sobject_describe", "Describe a Salesforce SObject from a project. This command retrieves detailed metadata about a specific Salesforce SObject, including its fields, relationships, and other properties. The results are returned in JSON format, providing a comprehensive view of the SObject's structure. Use this command to understand the schema of a specific SObject, which is especially useful when writing Apex code or SOQL queries that interact with that SObject. Always execute this tool before querying or manipulating records in the SObject to ensure you have the correct field names and types.", {
        input: z.object({
            sourcePath: z
                .string()
                .describe("Absolute path to the Salesforce DX project directory (must contain .sf/config.json)"),
        ...
        const result = await executeSObjectDescribe(sourcePath, sObjectName, targetOrg);
        return {
            content: [
                {
                    type: "text",
