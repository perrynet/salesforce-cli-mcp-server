...
const schemaGenerateTabInputSchema = z.object({
    ...
    directory: z
        .string()
        .describe("Path to a 'tabs' directory that will contain the source files"),
    icon: z
        .number()
        .min(1)
        .max(100)
        .default(1)
        .describe("Number from 1 to 100 that specifies the color scheme and icon for the custom tab"),
});
const schemaGenerateTab = async (input) => {
...
export const registerSchemaTools = (server) => {
    server.tool("schema_generate_tab", "Generate metadata source files for a new custom tab on a custom object from a project. Custom tabs display custom object data in Salesforce navigation.", {
