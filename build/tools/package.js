...
const executePackageInstall = async (targetOrg, packageId, wait = 0, installationKey, publishWait = 0, apexCompile = "all", securityType = "AdminsOnly", upgradeType = "Mixed", apiVersion) => {
...
const executePackageUninstall = async (targetOrg, packageId, wait = 0, apiVersion) => {
...
export function registerPackageTools(server) {
    server.tool("package_install", "Install or upgrade a package version in a Salesforce org. Supports both package IDs (04t) and aliases with various configuration options.", {
        input: z.object({
            targetOrg: z
                .string()
        ...
        try {
            const result = await executePackageInstall(input.targetOrg, input.packageId, input.wait, input.installationKey, input.publishWait, input.apexCompile, input.securityType, input.upgradeType, input.apiVersion);
            return {
                content: [
                    {
                        type: "text",
    ...
    server.tool("package_uninstall", "Uninstall a second-generation package from the target org. Specify the package ID (starts with 04t) or alias for the package to uninstall.", {
        input: z.object({
            targetOrg: z
                .string()
                .describe("Username or alias of the target org. Not required if the 'target-org' configuration variable is already set."),
